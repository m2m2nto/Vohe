...
///
/// The address and the username are plain preferences; the token signing in
/// returns is a credential and lives in the keychain. The password is never
/// stored anywhere. Nothing here is required — with no address or no token the
/// app simply never talks to a backend and behaves exactly as it always did.
struct BackendSettings: Equatable {
    ...
    static let usernameKey = "vohe.backendUsername"
    ...
    static let empty = BackendSettings(address: "", username: "", token: "")
    ...
    /// Until the user saves Server settings once, the build-time address stands
    /// in — so a fresh install has one less thing to type.
    ...
    static func save(_ settings: BackendSettings) -> Bool {
        ...
        UserDefaults.standard.set(settings.username, forKey: usernameKey)
